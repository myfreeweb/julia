...
(define (lambda-optimize-vars! lam)
  ...
  (let* ((vi     (car (lam:vinfo lam)))
    ...
    (define (kill)
      ...
      (set! live (table)))
    (define (visit e)
      (cond ((and (pair? e) (lambda-opt-ignored-exprs (car e)))
             #t)
            ...
            ((and (pair? e) (eq? (car e) 'return))
             (visit (cadr e))
             (kill))
            ((and (pair? e) (memq (car e) '(break label symboliclabel symbolicgoto)))
             (kill))
            ((and (pair? e) (memq (car e) '(if elseif _while _do_while trycatch tryfinally)))
             (for-each (lambda (e)
                         (visit e)
                         (kill))
                       (cdr e)))
            (else
             ...
             (cond ((and (pair? e)
                   ...
                   (else
                    ...
                    #t)
                   #;((or (atom? e)
                        (memq (car e) '(= method new call foreigncall cfunction |::| &)))
                    #t)
                   #;(else
                    (error (string "unexpected expression head \"" (car e) "\"")))))))
    (visit (lam:body lam))
    (for-each (lambda (v)
                (if (has? seen v)
                    (let ((vv (assq v vi)))
